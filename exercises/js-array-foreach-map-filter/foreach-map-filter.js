...
function vowelCount(str){
    ...
    splitArr.forEach(function(index){
        ...
        if(vowels.indexOf(letter) !== -1){ //if current index(letter) is a vowel
            ...
            if(obj[letter]){//if current index(vowel) is already defined
                obj[letter]++; //++ on the value of the vowel
            }
            else { //if current index is undefined
                obj[letter] = 1; //create new key for vowel and set value to 1
            }
        }
    })
    return obj;
...
function removeVowels(str) {
    ...
    return str.toLowerCase().split("").filter(function(val) {
        return vowels.indexOf(val) === -1; //if the letter doesnt exist in vowels
      }).join("");
}
